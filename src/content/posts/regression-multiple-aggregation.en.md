...
Makes sense, right? It talks. It reads documents. It answers questions. It writes code. It generates images. And you think — I don't even know how I speak, how my brain works, how I make decisions. So a machine that does all of that… must be something completely out of reach, reserved for engineers with PhDs.
...
It's exactly what you do when you pick a restaurant in the evening. You check the reviews, the price, the distance, whether your friends liked it. You add all of that up in your head — and you decide. You just did AI without knowing it.
...
- They check the weather: it's -8°C outside.
- They check the calendar: it's a Monday, and there's a big match on TV tonight.
- They check the time: it's peak hour.
...
Let's stay in the power plant. You now want to predict the electricity consumption of the neighbourhood tonight. You have several pieces of information:
...
They are weights — coefficients the machine adjusts automatically from thousands of past examples. It learns that temperature matters a lot, that peak hour does too, that Sunday is very different from Monday.
:::

This is not magic. It's optimisation: **finding the weights that minimise the gap between what it predicts and what actually happened** — just like you adjust a recipe until it's right.
...
Linear regression is the basic tool for this kind of weighted aggregation. It's simple, fast, and often surprisingly effective for predicting continuous values.
:::
...
model.fit(X, y)  # we show it the data (X) and results (y) so it can learn the weights (w1, w2, w3, w4)
...
- [!] Answer `YES` or `NO` — and that's where we need to go one step further.
...
In real AI applications, many questions are binary — is this email spam? Will this flight be delayed? Is this transaction fraudulent? If we used our equation directly, we'd get values like `1.7` or `-0.3`. Hard to turn that into a clear decision. And impossible to talk in terms of probabilities.
...
It's called the {purple}sigmoid{/purple}. And once you've seen it, you'll spot it everywhere.
