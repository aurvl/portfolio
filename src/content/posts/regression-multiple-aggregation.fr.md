...
Dans cet article, on pose la première brique. La base. Pas de jargon, pas de maths complexes, juste l'intuition fondamentale sur laquelle tout le reste est construit.
...
Ce sont des poids, des coefficients que la machine ajuste automatiquement à partir de milliers d'exemples passés. Elle apprend que la température compte beaucoup, que l'heure de pointe aussi, et que le dimanche est très différent du lundi.
:::

Ce n'est pas de la magie. C'est de l'optimisation : **trouver les poids qui minimisent l'écart entre ce qu'elle prédit et ce qui s'est vraiment passé** - comme toi quand tu ajustes une recette jusqu'à ce qu'elle soit juste.

:::panel{tone="blue" title="L'analogie du coach"}
Un entraîneur qui évalue ses joueurs note chacun sur la vitesse, la qualité de passe, et la résistance physique. Il multiplie chaque note par son importance :

- la vitesse compte double en attaque
- la qualité de passe compte pour `1,5`
- la résistance physique compte pour `1`

Il additionne le tout pour obtenir un score final.

- [v] C'est ce qu'on appelle la *régression linéaire* : apprendre quels facteurs comptent, et combien.
:::

:::panel{tone="green" title="Régression linéaire : un outil de base"}
La régression linéaire est l'outil de base pour faire ce genre d'agrégation pondérée. C'est simple, rapide, et souvent très efficace pour prédire des valeurs continues.
:::
...
# Entraîner le modèle
model = LinearRegression()  # machine qui apprend la configuration optimale pour faire de bonnes prédictions
model.fit(X, y)  # on lui montre les données et le résultat pour qu'elle apprenne les poids
...
- [v] Une bonne baseline - toujours tester ça avant de complexifier
:::
